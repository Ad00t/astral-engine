...
glm::dvec3 vecToBody(RigidBody* rb, glm::dvec3 global) {
    return glm::inverse(glm::dquat(rb->rot)) * global;
}

glm::dvec3 vecToGlobal(RigidBody* rb, glm::dvec3 body) {
...
Controller::Controller(double sensorNoise)
    : sensorNoise(sensorNoise) {}

StarshipController::StarshipController(double sensorNoise) : Controller(sensorNoise) {
    thrusters.emplace("starship:main_engine", std::make_unique<Thruster>(
...
void StarshipController::updateSensors(RigidBody* rb) {
...
void StarshipController::runControlLoop(RigidBody* rb, double dT) {
    std::unique_ptr<Thruster>& me = thrusters.at("starship:main_engine");
    me->thrust = 0.0;
